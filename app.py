    ...
    if results.pose_landmarks:
        ...
        if not setup_done:
            ...
            if (head_top < nose_y < head_bottom) and (foot_top < left_ankle_y < foot_bottom) and (foot_top < right_ankle_y < foot_bottom):
                setup_done = True
                initial_hip_y = (left_hip[1] + right_hip[1])/2
                initial_ankle_y = (left_ankle_y + right_ankle_y)//2
        else:
            # -------------------------
            # Sit-up detection
            # -------------------------
            ...
            torso_angle = calculate_angle(
                ...
                ((left_shoulder[0]+right_shoulder[0])/2, shoulder_y), 
                (nose[0], nose[1])
            )

            down_thresh = initial_hip_y + 0.05
            up_thresh = initial_hip_y - 0.2

            # Stage detection with debounce
            if hip_y > down_thresh and torso_angle < 70:
                down_frames += 1
                up_frames = 0
                if down_frames >= 3:
                    situp_stage = "down"
            elif hip_y < up_thresh and torso_angle > 100 and situp_stage == "down":
                up_frames += 1
                down_frames = 0
                if up_frames >= 3:
                    situp_stage = "up"
